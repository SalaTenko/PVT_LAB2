...
uint64_t hpctimer_timer_get_freq(hpctimer_t *timer) { return timer->freq; }
...
double hpctimer_getwtime() {
  return (double)hpctimer_gettime() / (double)hpctimer_global_timer.freq * 1E-6;
}
